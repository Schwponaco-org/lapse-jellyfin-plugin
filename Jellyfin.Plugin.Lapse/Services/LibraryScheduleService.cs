...
public sealed class LibraryScheduleService : IHostedService, IDisposable
{
    ...
    private void OnTick(object? state)
    {
        ...
        foreach (var library in plugin.Configuration.Libraries)
        {
            ...
            if (!IsDue(library.ScheduleDay, library.GetScheduleTimeOfDay(), library.LastScheduledRunUtc, now))
            {
                continue;
            }
            ...
            _logger.LogInformation(
                "Scheduled sync for library {Library} queuing {Count} items",
    ...
    private static bool IsDue(DayOfWeek? day, TimeSpan timeOfDay, DateTime? lastRunUtc, DateTime now)
    {
        ...
        var since = now.TimeOfDay - timeOfDay;
        ...
        if (lastRunUtc.HasValue && DateTime.UtcNow - lastRunUtc.Value < TimeSpan.FromHours(12))
        {
            return false;
        }
